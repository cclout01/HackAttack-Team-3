...
import type { Route } from "./+types/PositonDetail";
import {
  ArrowLeft,
  MapPin,
  Clock,
...
import { MOCK_POSITIONS, MOCK_ORGS } from "app/data/mockData";
...
export async function loader({ params }: Route.LoaderArgs) {
  const position = MOCK_POSITIONS.find((p) => p.id === params.id);

  if (!position) {
    throw new Response("Position not found", { status: 404 });
  }

  const org = MOCK_ORGS.find((o) => o.id === position.orgId);
...
export default function PositionDetail() {
  ...
  const { position, org } = useLoaderData<typeof loader>();
  const {
    isSignedUp,
    signUpForPosition,
  ...
  const canSignUp =
    userMode === "volunteer" && !isPast && position.spotsAvailable > 0;

  return (
    <div style={{ backgroundColor: "#f5f0e8", minHeight: "100vh" }}>
      <div
        style={{ backgroundColor: "#fff9f2", borderBottom: "1px solid #e8ddd0" }}
      >
          ...
          <div className="lg:col-span-2 space-y-5">
            <div className="relative rounded-3xl overflow-hidden h-72">
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(to bottom, transparent 50%, rgba(45,80,22,0.7) 100%)",
                }}
              />
            ...
            >
              ...
              >
                ...
                <InfoItem icon={<Calendar size={14} />} label="Date">
                  {new Date(position.date + "T12:00:00").toLocaleDateString(
                    "en-US",
                    {
                      weekday: "long",
                      month: "long",
                ...
                <InfoItem icon={<Users size={14} />} label="Spots Available">
                  <span
                    style={{
                      color:
                        position.spotsAvailable === 0
                          ? "#c0392b"
                          : position.spotsAvailable < 4
                          ? "#d4813a"
                          : "#4a7c59",
                      fontWeight: 700,
                    }}
                  ...
                  of {position.spotsTotal} remaining
                </InfoItem>
              </div>
            </div>
            ...
            >
              ...
              {isCompleted ? (
                ...
                <button
                  onClick={() =>
                    signedUp
                      ? withdrawFromPosition(position.id)
                      : signUpForPosition(position.id)
                  }
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p
                    className="text-sm"
                    style={{ color: "#2c1e12", fontWeight: 700 }}
                  >
                    {org.name}
                  </p>
              ...
              <p
                ...
                style={{ color: "#6b5040", lineHeight: 1.6 }}
              >
                {org.description}
              </p>
              <a
